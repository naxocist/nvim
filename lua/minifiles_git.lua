...
-- Requires Neovim 0.10+ and mini.files.
  ...
  vim.schedule(function()
    ...
    for i = 1, nlines do
      ...
      if status then
        ...
        -- This below code is responsible for coloring the text of the items.
...
-- Thanks for the idea of getting signs for dirs from
...
-- Scan every column buffer (focused folder AND its parents), not just the
-- current one. MiniFilesBufferCreate fires once per column; the git call is
-- cached per repo so this is still one `git status` per repo.
autocmd("User", {
  group = augroup("start"),
